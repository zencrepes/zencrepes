...
class FetchOrgRepos extends Component {
      ...
      if (this.errorRetry <= 3) {
        ...
        if (data.data !== null && data.data !== undefined) {
          ...
          if (data.data.organization !== null) {
            ...
            log.info(
              "Total count: " + data.data.organization.repositories.totalCount
            );
  ...
  loadRepositories = async data => {
    const { incrementLoadedRepos, log, setLoadingMsg } = this.props;
    let lastCursor = null;
...
export default connect(
  mapState,
  mapDispatch
)(withApollo(withSnackbar(FetchOrgRepos)));
