                    ...
                    <Grid item xs={12} sm container>
                        ...
                        >
                            <Grid item >
                                ...
                                >
                                    <Grid item >
                                        <a href={repository.url} className={classes.repoName} target="_blank" rel="noopener noreferrer">{repository.org.login}/{repository.name}</a>
                                    </Grid>
                                </Grid>
                            </Grid>
                            <Grid item >
                                        ...
                                        repository.languages.edges.map((language) => {
                                            return (
                                                <Grid item key={language.node.name} >
                                                    <Label size="small" m={1} style={{background: "#" + language.node.color, color: fontColorContrast("#" + language.node.color)}}>{language.node.name}</Label>
                                                </Grid>
                                            )
                                        })
                                    }
                            ...
                            <Grid item className={classes.repositorySubTitle}>
                                <span><a href={repository.url} className={classes.repositorySubTitle} target="_blank" rel="noopener noreferrer">#{repository.number}</a> </span>
                                <span>created on <Moment format="ddd MMM D, YYYY">{repository.createdAt}</Moment></span>
                                <span>, last update on <Moment format="ddd MMM D, YYYY">{repository.updatedAt}</Moment></span>                                
                                <span>, last push on <Moment format="ddd MMM D, YYYY">{repository.pushedAt}</Moment></span>
                            </Grid>
                        </Grid>
                    </Grid>
                </Grid>
