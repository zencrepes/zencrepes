...
Tracker.autorun(function() {
  if (Meteor.user()) {
    ...
    localCfgSources.refresh(true, () => {
      store.dispatch.startup.setLoadedSources(true);
    });

    const localCfgIssues = new PersistentMinimongo2(
      cfgIssues,
      "GAV-Issues-" + username
    );
    localCfgIssues.refresh(true, () => {
      store.dispatch.startup.setLoadedIssues(true);
    });

    const localCfgPullrequests = new PersistentMinimongo2(
      cfgPullrequests,
      "GAV-Pullrequests-" + username
    );
    localCfgPullrequests.refresh(true, () => {
      store.dispatch.startup.setLoadedPullrequests(true);
    });

    const localCfgQueries = new PersistentMinimongo2(
      cfgQueries,
      "GAV-Queries-" + username
    );
    localCfgQueries.refresh(true, () => {
      store.dispatch.startup.setLoadedQueries(true);
    });

    const localCfgLabels = new PersistentMinimongo2(
      cfgLabels,
      "GAV-Labels-" + username
    );
    localCfgLabels.refresh(true, () => {
      store.dispatch.startup.setLoadedLabels(true);
    });

    const localCfgMilestones = new PersistentMinimongo2(
      cfgMilestones,
      "GAV-Milestones-" + username
    );
    localCfgMilestones.refresh(true, () => {
      store.dispatch.startup.setLoadedMilestones(true);
    });

    const localCfgProjects = new PersistentMinimongo2(
      cfgProjects,
      "GAV-Projects-" + username
    );
    localCfgProjects.refresh(true, () => {
      store.dispatch.startup.setLoadedProjects(true);
    });

    const localCfgRepositories = new PersistentMinimongo2(
      cfgRepositories,
      "GAV-Repositories-" + username
    );
